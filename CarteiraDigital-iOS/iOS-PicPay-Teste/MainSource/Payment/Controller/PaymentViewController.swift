...
class PaymentViewController: BaseTableViewCStatic {
    ...
    @objc fileprivate func goToChecking(_ sender: DefaultButton) {
        ...
        pay.PaymentRequest { [weak self] (error: String?, response: TicketUser?) in
            ...
            if result.transaction.success {
                self?.didPaymentSuccess(result)
            } else {
                self?.didPaymentFailure(result.transaction.status)
            }
        }
    }
...
extension PaymentViewController: UITextFieldDelegate {
    ...
    func didPaymentSuccess(_ ticketU: TicketUser) {
        ...
        data.save()
        ControlNewPayment.didPaymentSuccess(with: ticketU)
        self.navigationController?.popToRootViewController(animated: true)
    }
    
    func didPaymentFailure(_ ticketU: String) {
