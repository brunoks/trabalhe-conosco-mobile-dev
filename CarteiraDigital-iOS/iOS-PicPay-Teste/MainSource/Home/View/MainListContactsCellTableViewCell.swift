...
class MainListContactsCell: UITableViewCell {

    static let cellId = "ListaCellID"
    ...
    var item: Contato! {
        didSet {
            let viewmodel = ContatosCellViewModel(contato: item)
            
            self.labelname.text = viewmodel.name
            self.labelnickname.text = viewmodel.username
            self.imgavatar.sd_setImage(with: URL(string: viewmodel.img), placeholderImage: UIImage(named: "noprofile.png"))
        }
    }
